...
# SPDX identifier -> the name a reader recognises. Only ids actually used across
# these repositories; anything else is refused rather than guessed at, because the
# alternative is a licence document naming a licence nobody chose.
#
# BUSL-1.1 is the Business Source License 1.1. It is emphatically not "BSL-1.1",
# which SPDX does not define, and not BSL-1.0, which is the Boost Software
# License -- a permissive licence, and close to the opposite of this one.
...
def project_license() -> tuple[str, str]:
    """This project's own licence, as (SPDX id, display name).

    Read from pyproject rather than written down here, and this is not
    fastidiousness: hardcoding it put "licensed under the Business Source
    License 1.1" at the top of an Apache-2.0 project's notice the first time this
    script was vendored somewhere new. A licence file asserting the wrong licence
    is the worst thing this script could produce, so the assertion is derived from
    the same field that governs packaging.
    """
    project = tomllib.loads((ROOT / "pyproject.toml").read_text(encoding="utf-8"))["project"]
    declared = project.get("license")
    # PEP 639 replaced the {text = "..."} table with a bare SPDX string; both are
    # still in the wild across these repositories.
    identifier = declared.get("text", "") if isinstance(declared, dict) else str(declared or "")
    identifier = identifier.strip()
    if not identifier:
        raise SystemExit(
            "error: pyproject declares no [project] license, so this notice cannot "
            "state what the project itself is under. Add one as an SPDX identifier."
        )
    if identifier not in LICENSE_DISPLAY_NAMES:
        raise SystemExit(
            f"error: pyproject declares license {identifier!r}, which this script has "
            f"no display name for. Add it to LICENSE_DISPLAY_NAMES rather than letting "
            f"the notice name a licence nobody chose — and check it is a real SPDX "
            f"identifier first: BUSL-1.1 is the Business Source License, while BSL-1.0 "
            f"is Boost and BSL-1.1 is not an SPDX identifier at all."
        )
    return identifier, LICENSE_DISPLAY_NAMES[identifier]
...
PROJECT_NAME = "mcp-xray"
ROOT_PACKAGE = "mcp-xray-audit"
MODULE_NAME = "mcp_xray"

# Whether a wheel and sdist are published. Gates the paragraph about them.
PUBLISHES_TO_PYPI = True

# No container image is published here — this ships as a wheel only. That makes
# the whole notice informational: pip fetches every dependency below from PyPI
# directly, so nothing we publish hands anyone a binary copy of them and none of
# the attribution duties that redistribution triggers apply. The file is still
# generated and still gated, because the licence check is worth running whether
# or not an obligation has attached yet — and because the day an image appears,
# this flag flips and the same list becomes a notice that is owed.
PUBLISHES_IMAGE = False

# The gate is kept, the artifact is not. Nothing here is redistributed, so a
# committed notice would be almost entirely licence texts nobody is owed,
# rewritten on every dependency bump. What earns its keep is the check that
# refuses a dependency whose terms nobody has reviewed — see ACKNOWLEDGED below,
# which exists because that check fired. Run the script without --check to
# produce the file for a look; it is gitignored rather than tracked.
COMMIT_NOTICE = False

# Every extra is accounted for even though none is redistributed: `api`, `openai`
# and `live` are what a user actually installs to audit a live server, so they
# belong in a list whose purpose is telling somebody what this pulls in. `lint`
# and `dev` are tooling the project runs, not works it ships.
NOTICED_EXTRAS: tuple[str, ...] = ("api", "openai", "live")

# Nothing to resolve: there is no Dockerfile.
IMAGE_BUILD_ARGS: dict[str, str] = {}

# Unused while PUBLISHES_IMAGE is False — the platform-layer section it feeds is
# a statement about an image, and there is none.
PLATFORM_LINKED_LIBS = "glibc, libffi,\n  libssl, libsqlite3, liblzma and readline"

# No pyphen in this closure — nothing to elect.
...
ACKNOWLEDGED: dict[str, Acknowledgement | Pending] = {
    ...
    "tqdm": Acknowledgement(
        "MPL-2.0 AND MIT",
        "MPL-2.0 is file-level copyleft and its obligations attach on "
        "distribution. mcp-xray distributes nothing: it publishes a wheel, and a "
        "wheel that declares a dependency hands over no copy of it — pip fetches "
        "tqdm from PyPI onto the user's own machine, under terms that are then "
        "between them and the tqdm authors. Nothing we publish contains tqdm's "
        "code, so there is no copy for the file-level condition to reach.\n\n"
        "This reasoning is narrower than the one that clears certifi elsewhere in "
        "this family, and deliberately so: it turns on there being no artifact "
        "rather than on how the artifact is built. It stops holding the moment "
        "mcp-xray publishes a container image or otherwise vendors its "
        "dependencies. If PUBLISHES_IMAGE above is ever set True, this entry has "
        "to be rewritten on the certifi footing — tqdm shipped unmodified as a "
        "separate installed package, obligation satisfied by carrying its licence "
        "text — or re-reviewed.",
    ),}
...
def render(packages: list[Package], image_extras: frozenset[str], lock: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        *paragraph(
            ...
            f"[{project_license()[1]}](LICENSE). This file covers the third-party "
            f"packages it depends on and reproduces the attribution each one requires."
        ),
        "",
        "## Scope",
        "",
